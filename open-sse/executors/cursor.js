...
function createRequestContextResponse() {
  // AgentService asks every run for client context. aimix has no IDE file
      ...
      try {
        while (!finished) {
          ...
          pending = decodeAgentFrames(pending, (payload) => {
            ...
            // Return an empty context; aimix is not coupled to an editor.
            if (serverMessage.has(2)) {
              ...
              if (execRequest.has(10)) {
                ...
                // (shell, read, write, …) that aimix cannot service. Fail the
