...
**AIMixは、サブスクリプションの価値を最大化し、コストを最小限に抑えるAIモデルルーターです。**
...
**AIMixはすべてのプロバイダーのクォータをリアルタイムで追跡します:**
...
**はい! AIMixは複数のデプロイメントオプションをサポートします:**
...
- `DATA_DIR` - データベース保存パス(デフォルト: `~/.aimix`)
...
**はい、AIMixはセキュリティとプライバシーを優先します:**

**ローカルストレージ:**
- すべてのデータは`~/.aimix`(またはカスタム`DATA_DIR`)にローカル保存
- AIMixサーバーへのデータ送信なし
- OAuthトークンはJWTで暗号化
