...
AIMixをCline VSCode拡張機能と統合し、AIリクエストをAIMixのインテリジェントルーティングシステム経由でルーティングします。

## 前提条件

- Visual Studio Codeがインストール済み
- VSCodeマーケットプレイスからCline拡張機能がインストール済み
- AIMixがローカルで動作中、またはクラウドエンドポイントが設定済み
...
**手順:**
1. **Base URL** フィールドにAIMixエンドポイントを入力
2. 末尾に `/v1` を必ず含める
...
3. AIMix経由でAIによる説明を取得

### ファイル操作

1. Clineにファイルの作成、変更、削除を依頼
2. ClineはAIMixを使用してコンテキストを理解し変更を加える
3. 受け入れる前に変更を確認
...
1. AIMixが動作中か確認: `curl http://localhost:20128/health`
...
localhostの代わりにAIMixクラウドエンドポイントを使用:

1. Cline設定で、Base URLを設定: `http://localhost:20128`
2. AIMixクラウドダッシュボードでAPIキーが設定されていることを確認
...
AIMixは以下に基づいて最適な利用可能なプロバイダーにリクエストを自動的にルーティング:
- モデル可用性
- プロバイダーヘルスステータス
- コスト最適化
- ロードバランシング
...
プロバイダーが失敗した場合、AIMixは自動的にダッシュボードで設定された代替プロバイダーにフォールバックします。

### 使用量トラッキング

AIMixダッシュボード経由でCline使用量をモニター:
- 総リクエスト数
- トークン使用量
- モデルごとのコスト
- プロバイダー分布
