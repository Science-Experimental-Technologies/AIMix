...
Despliega AIMix en VPS o Docker para acceso remoto y uso en producción.
...
| `DATA_DIR` | `~/.aimix` | Ruta de almacenamiento de la base de datos |
...
```bash
sudo mkdir -p /var/lib/aimix
sudo chown $USER:$USER /var/lib/aimix
```
...
Crea `/etc/nginx/sites-available/aimix`:
...
sudo ln -s /etc/nginx/sites-available/aimix /etc/nginx/sites-enabled/
...
# Si NO usas proxy reverso, permite los puertos de AIMix
...
tar -czf aimix-backup-$(date +%Y%m%d).tar.gz /var/lib/aimix

# Respaldo automatizado diario (agregar a crontab)
0 2 * * * tar -czf /backups/aimix-$(date +\%Y\%m\%d).tar.gz /var/lib/aimix
```
...
```bash
# Verificar si AIMix está corriendo
pm2 status
...
sudo chown -R $USER:$USER /var/lib/aimix
chmod 755 /var/lib/aimix
```
