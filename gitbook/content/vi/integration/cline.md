...
Tích hợp AIMix với extension Cline VSCode để định tuyến request AI qua hệ thống routing thông minh của AIMix.

## Yêu cầu

- Visual Studio Code đã cài đặt
- Extension Cline đã cài đặt từ VSCode marketplace
- AIMix đang chạy cục bộ hoặc cloud endpoint đã cấu hình
...
Đặt base URL tới endpoint AIMix:
...
3. Nhấn Enter để gửi
4. Cline sẽ dùng AIMix để xử lý request

### Tạo Code

1. Yêu cầu Cline tạo code: "Create a React component for a login form"
2. Cline sẽ tạo code qua AIMix
...
3. Nhận giải thích AI qua AIMix

### Thao tác File

1. Yêu cầu Cline tạo, sửa hoặc xóa files
2. Cline sẽ dùng AIMix để hiểu context và thực hiện thay đổi
...
1. Xác minh AIMix đang chạy: `curl http://localhost:20128/health`
...
2. Xác minh AIMix instance đang chạy và healthy
3. Thử reload cửa sổ VSCode (Cmd/Ctrl + Shift + P → "Reload Window")
4. Kiểm tra logs AIMix để xem lỗi

## Cấu hình Nâng cao

### Dùng Cloud Endpoint

Để dùng AIMix cloud endpoint thay vì localhost:

1. Trong Cline settings, đặt Base URL: `http://localhost:20128`
2. Đảm bảo bạn đã cấu hình API key trong AIMix cloud dashboard
...
## Tích hợp với Tính năng AIMix

### Định tuyến Model

AIMix tự động định tuyến request đến provider tốt nhất hiện có dựa trên:
...
Nếu một provider thất bại, AIMix tự động fallback sang provider khác đã cấu hình trong dashboard.

### Theo dõi Usage

Giám sát usage Cline qua AIMix dashboard:
- Tổng request
- Token usage
- Chi phí mỗi model
- Phân bổ provider
