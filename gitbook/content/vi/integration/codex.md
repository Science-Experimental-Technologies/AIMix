...
Tích hợp AIMix với OpenAI Codex CLI để định tuyến request API OpenAI qua hệ thống routing thông minh của AIMix.
...
AIMix cung cấp các model Codex sau:
...
1. Xác minh API key đúng trong AIMix dashboard
...
1. Xác minh tên model khớp với cấu hình AIMix
2. Kiểm tra kết nối provider OpenAI đang hoạt động trong AIMix dashboard
...
Đảm bảo bạn đã cấu hình API key trong AIMix cloud dashboard.
