<div align="center">
  <img src="../images/aimix.png?1" alt="Панель управления AIMix" width="800"/>

  # AIMix - Free AI Router
...
Пакет этого репозитория приватный (`aimix-app`), поэтому запуск из исходников/Docker — это ожидаемый путь локальной разработки.
...
### 📺 Полное руководство по настройке - AIMix + Claude Code БЕСПЛАТНО

[![Настройка AIMix + Claude Code](https://img.youtube.com/vi/raEyZPg5xE0/maxresdefault.jpg)](https://www.youtube.com/watch?v=raEyZPg5xE0)

**🎬 Полное пошаговое руководство:**
- ✅ Установка и настройка AIMix
...
AIMix бесшовно работает со всеми основными AI-инструментами для кодинга:
...
- Ваш CLI-инструмент отправляет формат OpenAI → AIMix транслирует → Провайдер получает родной формат
...
> Сам AIMix **никогда ничего не взимает** с вас. Вы платите напрямую провайдерам (если используете платные сервисы).
>
...
### 📊 Понимание затрат и оплаты в AIMix

**Реальность оплаты AIMix:**

✅ **Софт AIMix = БЕСПЛАТНО навсегда** (открытый код, никогда не взимает плату)
...
❌ **AIMix никогда не выставляет счёт** и не списывает с вашей карты
...
- **Дешёвые провайдеры** (GLM, MiniMax): Платите им напрямую, AIMix только маршрутизирует
- **БЕСПЛАТНЫЕ провайдеры** (iFlow, Kiro, Qwen): Действительно бесплатны навсегда, без скрытых платежей
- **AIMix**: Никогда ничего не взимает, никогда
...
Панель отслеживает ваше использование токенов и показывает **оценочные затраты**, как если бы вы напрямую использовали платный API. Это **не реальная оплата** — это справка, показывающая, сколько вы экономите, используя бесплатные модели или существующие подписки через AIMix.
...
<details>
<summary><b>💳 Взимает ли с меня плату AIMix?</b></summary>

**Нет.** AIMix — это бесплатное ПО с открытым кодом, работающее на вашем собственном компьютере. Оно никогда ничего с вас не взимает.

**Вы платите только:**
- ✅ **Провайдерам подписки** (Claude Code $20/мес, Codex $20-200/мес) → Платите им напрямую на их сайте
- ✅ **Дешёвым провайдерам** (GLM, MiniMax) → Платите им напрямую, AIMix только маршрутизирует ваши запросы
- ❌ **Самому AIMix** → **Никогда ничего не взимает, никогда**

AIMix — это локальный прокси/роутер. У него нет вашей кредитной карты, он не может выставлять счета и не имеет платёжной системы. Это полностью бесплатное ПО.
...
AIMix только маршрутизирует ваши запросы к ним — никаких «ловушек» или будущих платежей. Это действительно бесплатные сервисы, а AIMix облегчает их использование с поддержкой резервирования.

**Примечание:** Некоторые провайдеры подписки (Antigravity, GitHub Copilot) могут иметь бесплатные пробные периоды, которые позже становятся платными, но об этом чётко уведомляют сами провайдеры, а не AIMix.
...
3. **Используйте провайдеров подписки в последнюю очередь:**
   ...
   - AIMix помогает максимизировать их ценность через отслеживание квоты
...
Умный механизм резервирования AIMix предотвращает неожиданные расходы:
...
**Вы контролируете:** Установите лимиты расходов на каждого провайдера в панели, и AIMix будет их соблюдать.
...
**Профи-совет:** Используйте Opus для сложных задач, Sonnet для скорости. AIMix отслеживает квоту для каждой модели!
...
Settings → Models → Advanced:
  ...
  OpenAI API Key: [из панели управления aimix]
...
> **Примечание:** OpenClaw работает только с локальным AIMix. Используйте `127.0.0.1` вместо `localhost`, чтобы избежать проблем с разрешением имён.
...
|----------|---------|-------------|
| `JWT_SECRET` | Автогенерация (`~/.aimix/jwt-secret`) | Секрет подписи JWT для cookie аутентификации панели (задайте для общего доступа между инстансами) |
| `INITIAL_PASSWORD` | `123456` | Пароль первого входа при отсутствии сохранённого хеша |
| `DATA_DIR` | `~/.aimix` | Расположение основной БД приложения (`db.json`) |
...
- История использования и логи: `~/.aimix/usage.json` и `~/.aimix/log.txt`, управляется `src/lib/usageDb.js`.
- Опциональные логи запросов/транслятора: `<repo>/logs/...` при `ENABLE_REQUEST_LOGS=true`.
- Хранилище использования следует логике пути `~/.aimix` и независимо от `DATA_DIR`.
...
- Локальный рантайм AIMix включает резервирование SSE→JSON для непотоковых вызовов, когда upstream возвращает `text/event-stream`.
...
Спасибо всем, кто помогает делать AIMix лучше!
