<div align="center">
  <img src="../images/aimix.png?1" alt="Bảng điều khiển AIMix" width="800"/>

  # AIMix - Free AI Router & Token Saver

  **Không bao giờ ngừng code. Tiết kiệm 20-40% token với RTK + tự động dự phòng sang các mô hình AI MIỄN PHÍ & giá rẻ.**

  **Kết nối tất cả công cụ AI Code (Claude Code, Codex, Cursor, Cline, Copilot, Antigravity...) tới 120+ Nhà cung cấp AI & 200+ Mô hình.**
...
Gói kho lưu trữ này là riêng tư (`aimix-app`), vì vậy việc chạy từ nguồn/Docker là cách phát triển cục bộ mặc định.
...
### 📺 Hướng dẫn thiết lập hoàn chỉnh - AIMix + Claude Code MIỄN PHÍ

[![Thiết lập AIMix + Claude Code](https://img.youtube.com/vi/raEyZPg5xE0/maxresdefault.jpg)](https://www.youtube.com/watch?v=raEyZPg5xE0)

**🎬 Xem hướng dẫn từng đầy đủ:**
- ✅ Cài đặt & thiết lập AIMix
...
AIMix hoạt động liền mạch với tất cả các công cụ code AI chính:
...
- Công cụ CLI của bạn gửi định dạng OpenAI → AIMix dịch chuyển → Nhà cung cấp nhận định dạng gốc
...
> Bản thân AIMix **không bao giờ thu phí** bạn bất cứ thứ gì. Bạn chỉ trả tiền trực tiếp cho các nhà cung cấp (nếu sử dụng dịch vụ trả phí).
>
...
### 📊 Hiểu về Chi phí & Thanh toán của AIMix

**Thực tế Thanh toán AIMix:**

✅ **Phần mềm AIMix = MIỄN PHÍ mãi mãi** (mã nguồn mở, không bao giờ thu phí)
...
❌ **AIMix không bao giờ gửi hóa đơn** hoặc tính phí thẻ của bạn
...
- **Nhà cung cấp giá rẻ** (GLM, MiniMax): Trả tiền trực tiếp cho họ, AIMix chỉ định tuyến
...
<details>
<summary><b>💳 Tôi có bị AIMix tính phí không?</b></summary>

**Không.** AIMix là phần mềm miễn phí, mã nguồn mở chạy trên máy tính của chính bạn. Nó không bao giờ tính phí bạn bất cứ thứ gì.

**Bạn chỉ trả tiền:**
- ✅ **Nhà cung cấp gói đăng ký** (Claude Code $20/tháng, Codex $20-200/tháng) → Trả tiền trực tiếp cho họ trên website của họ
- ✅ **Nhà cung cấp giá rẻ** (GLM, MiniMax) → Trả tiền trực tiếp cho họ, AIMix chỉ định tuyến yêu cầu của bạn
- ❌ **Bản thân AIMix** → **Không bao giờ tính phí bất cứ thứ gì, ever**

AIMix là một proxy/router cục bộ. Nó không cóẻ tín dụng của bạn, không thể gửi hóa đơn và không có hệ thống thanh toán. Đó là phần mềm hoàn toàn miễn phí.
...
AIMix chỉ định tuyến yêu cầu của bạn đến họ - không có "cạm bẫy" hay thanh toán trong tương lai. Đó là các dịch vụ thực sự miễn phí, và AIMix giúp chúng dễ sử dụng với hỗ trợ dự phòng.

**Lưu ý:** số nhà cung cấp gói đăng ký (Antigravity, GitHub Copilot) có thể có các khoảng thời gian dùng thử miễn phí có thể trở thành trả phí sau này, nhưng điều này sẽ được các nhà cung cấp đó thông báo rõ ràng, không phải AIMix.
...
3. **Sử dụng nhà cung cấp gói đăng ký cuối cùng:**
   ...
   - AIMix giúp tối đa hóa giá trị của chúng thông qua theo dõi hạn mức
...
Cơ chế dự phòng thông minh của AIMix ngăn chặn các khoản phí bất ngờ:
...
**Bạn nắm quyền kiểm soát:** Đặt giới hạn chi tiêu cho mỗi nhà cung cấp trong bảng điều khiển, và AIMix sẽ tuân thủ chúng.
...
**Mẹo Chuyên nghiệp:** Sử dụng Opus cho các tác vụ phức tạp, Sonnet cho tốc độ. AIMix theo dõi hạn mức cho mỗi mô hình!
...
Settings → Models → Advanced:
  ...
  OpenAI API Key: [từ bảng điều khiển aimix]
...
> **Lưu ý:** OpenClaw chỉ hoạt động với AIMix cục bộ. Sử dụng `127.0.0.1` thay vì `localhost` để tránh các vấn đề phân giải6.
...
docker run -d \
  --name aimix \
  -p 20128:20128 \
  --env-file /root/dev/aimix/.env \
  -v aimix-data:/app/data \
  -v aimix-usage:/root/.aimix \
  aimix
```
...
| Biến | Mặc định | Mô tả |
|----------|---------|-------------|
| `JWT_SECRET` | Tự động sinh (`~/.aimix/jwt-secret`) | Bí mật ký JWT cho cookie xác thực bảng điều khiển (đặt để chia sẻ giữa nhiều instance) |
| `INITIAL_PASSWORD` | `123456` | Mật khẩu đăng nhập đầu tiên khi không có hash đã lưu tồn tại |
| `DATA_DIR` | `~/.aimix` |ị trí cơ sở dữ liệu ứng dụng chính (`db.json`) |
...
| `CLOUD_URL` | `http://localhost:20128` | URL cơ sở endpoint đồng bộ đám mây phía máy chủ |
| `NEXT_PUBLIC_BASE_URL` | `http://localhost:3000` | URL cơ sở tương thích ngược/công khai (ưu tiên `BASE_URL` cho runtime máy chủ) |
| `NEXT_PUBLIC_CLOUD_URL` | `http://localhost:20128` | URL đám mây tương thích ngược/công khai (ưu tiên `CLOUD_URL` cho runtime máy chủ) |
...
- Lịch sử sử dụng và log: `~/.aimix/usage.json` và `~/.aimix/log.txt`, được quản lý bởi `src/lib/usageDb.js`.
- request/translator tùy chọn: `<repo>/logs/...` khi `ENABLE_REQUEST_LOGS=true`.
- Lưu trữ sử dụng hiện tại tuân theo logic đường dẫn `~/.aimix` và độc lập với `DATA_DIR`.
...
- Xác minh `CLOUD_URL` trỏ đến endpoint đám mây dự kiến của bạn (ví dụ: `http://localhost:20128`)
...
- Triệu chứng thường xuất hiện trên endpoint đám mây công khai (`http://localhost:20128/v1`) cho các lệnh gọi không phát trực tiếp (non-streaming).
...
- Runtime AIMix cục bộ bao gồm dự phòng SSE→JSON cho các lệnh gọi không phát trực tiếp khi upstream trả về `text/event-stream`.
...
- Endpoint đám mây công khai (`http://localhost:20128/v1/chat/completions`):
...
Cảm ơn tất cả những người đã đóng góp giúp AIMix tốt hơn!
