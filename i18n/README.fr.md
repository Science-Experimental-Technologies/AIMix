<div align="center">
  <img src="../images/aimix.png?1" alt="Tableau de bord AIMix" width="800"/>

  # AIMix - Routeur IA GRATUIT et économiseur de tokens
...
[🚀 Démarrage rapide](#-démarrage-rapide) • [💡 Fonctionnalités](#-fonctionnalités-clés) • [📖 Configuration](#-guide-dinstallation) • [🌐 Site web](http://localhost:20128)
...
Le paquet de ce dépôt est privé (`aimix-app`), donc l'exécution depuis le code source/Docker est le chemin de développement local attendu.
...
<table>
  ...
  <b>🇻🇳 Tiếng Việt</b><br/>
  <sub>Tiết kiệm chi phí LLM cho OpenClaw với AIMix<br/>by <a href="https://www.youtube.com/c/M%C3%ACAIblog">Mì AI</a></sub>
</td>
<td align="center" width="320">
      ...
      <b>🇵🇰 اردو / हिन्दी</b><br/>
      <sub>AIMix + Claude Code availability varies Setup<br/>by <a href="https://www.youtube.com/@BuildAIWithHamid">Build AI With Hamid</a></sub>
    </td>
    <td align="center" width="320">
      ...
      <b>🇺🇸 English</b><br/>
      <sub>AIMix + Claude Code FREE Setup<br/>by <a href="https://www.youtube.com/@BuildAIWithHamid">Build AI With Hamid</a></sub>
    </td>
  ...
  <td align="center" width="320">
    ...
    <td align="center" width="320">
      ...
      <b>🇺🇸 English</b><br/>
      <sub>Claude CLI Free Setup with AIMix 🚀<br/>by <a href="https://www.youtube.com/@CodeVerseSoban">CodeVerse Soban</a></sub>
    </td>
  ...
  <td align="center" width="320">
      ...
      <b>🇻🇳 Tiếng Việt</b><br/>
      <sub>Cài Đặt OpenClaw Free Từ A-Z + AIMix<br/>by <a href="https://www.youtube.com/@maigia">Mai Gia</a></sub>
    </td>
    <td align="center" width="320">
    ...
    <td align="center" width="320">
      ...
      <b>🇮🇩 Indonesia</b><br/>
      <sub>Koding 24 Jam Anti Rate Limit! Hemat Token AI 65% | Tutorial Quick Setup AIMix 🚀<br/>by <a href="https://www.youtube.com/@krisswuh">Krisswuh</a></sub>
    </td>
  ...
  <td align="center" width="320">
      ...
      <b>🇮🇩 Indonesia</b><br/>
      <sub>Cara Deploy AIMix di Hugging Face GRATIS Non-Stop! | Alternatif VPS RAM 16GB<br/>by <a href="https://www.youtube.com/@krisswuh">Krisswuh</a></sub>
    </td>
  </tr>
...
AIMix fonctionne parfaitement avec tous les principaux outils de codage IA :
...
Headroom est optionnel et s'exécute séparément. AIMix appelle l'endpoint local `/v1/compress` de Headroom, puis conserve le routage, le repli, l'authentification et le suivi d'utilisation normaux :

```
Client → AIMix → Headroom /v1/compress → AIMix → fournisseur
```
...
Si Headroom est indisponible ou renvoie une erreur, AIMix bascule en mode dégradé et envoie la requête originale.
...
- Votre outil CLI envoie le format OpenAI → AIMix le traduit → Le fournisseur reçoit le format natif
...
> AIMix lui-même **ne vous facture jamais** quoi que ce soit. Vous ne payez que les fournisseurs directement (si vous utilisez des services payants).
>
...
### 📊 Comprendre les coûts et la facturation de AIMix

**La réalité de la facturation de AIMix :**

✅ **Le logiciel AIMix = GRATUIT pour toujours** (open source, ne facture jamais)
...
❌ **AIMix n'envoie jamais de factures** et ne débite pas votre carte
...
- **Fournisseurs bon marché** (GLM, MiniMax) : Payez-les directement, AIMix se contente de router
- **Fournisseurs GRATUITS** (iFlow, Kiro, Qwen) : Vraiment gratuits pour toujours, sans frais cachés
- **AIMix** : Ne facture jamais rien, jamais
...
Le tableau de bord suit votre utilisation de tokens et affiche des **coûts estimés** comme si vous utilisiez des API payantes directement. Ce n'est **pas une facturation réelle** — c'est une référence pour montrer combien vous économisez en utilisant des modèles gratuits ou des abonnements existants via AIMix.
...
<details>
<summary><b>💳 AIMix me facturera-t-il ?</b></summary>

**Non.** AIMix est un logiciel gratuit et open source qui s'exécute sur votre propre ordinateur. Il ne vous facture jamais rien.

**Vous ne payez que :**

- ✅ **Fournisseurs par abonnement** (Claude Code 20 $/mois, Codex 20 à 200 $/mois) → Payez-les directement sur leurs sites web
- ✅ **Fournisseurs bon marché** (GLM, MiniMax) → Payez-les directement, AIMix route simplement vos requêtes
- ❌ **AIMix lui-même** → **Ne facture jamais rien, jamais**

AIMix est un proxy/routeur local. Il n'a pas votre carte bancaire, ne peut pas envoyer de factures et ne possède aucun système de facturation. C'est un logiciel entièrement gratuit.
...
AIMix se contente de router vos requêtes vers eux — il n'y a pas de "piège" ni de facturation future. Ce sont de véritables services gratuits, et AIMix les rend faciles à utiliser avec le support du repli.
...
3. **Utilisez les fournisseurs par abonnement en dernier :**
   ...
   - AIMix vous aide à maximiser leur valeur grâce au suivi de quota
...
Le repli intelligent de AIMix évite les frais surprises :
...
**Vous gardez le contrôle :** Définissez des limites de dépenses par fournisseur dans le tableau de bord, et AIMix les respecte.
...
**Astuce pro :** Utilisez Opus pour les tâches complexes et Sonnet pour la vitesse. AIMix suit le quota par modèle !
...
Settings → Models → Advanced :
  ...
  OpenAI API Key : [depuis le tableau de bord aimix]
...
```json
{
  ...
  "anthropic_api_key": "votre-clé-api-aimix"
}
```
...
export OPENAI_API_KEY="votre-clé-api-aimix"
...
> **Remarque :** OpenClaw ne fonctionne qu'avec un AIMix local. Utilisez `127.0.0.1` au lieu de `localhost` pour éviter les problèmes de résolution IPv6.
...
**Persistance des données :** `$HOME/.aimix/db/data.sqlite` sur l'hôte ↔ `/app/data/db/data.sqlite` dans le conteneur.

### Variables d'environnement

| Variable                                             | Par défaut                                  | Description                                                                                  |
| ---------------------------------------------------- | ------------------------------------------- | -------------------------------------------------------------------------------------------- |
| `JWT_SECRET`                                         | Généré automatiquement (`~/.aimix/jwt-secret`) | Secret de signature JWT pour le cookie d'authentification du tableau de bord (remplacez-le pour partager entre instances) |
| `INITIAL_PASSWORD`                                   | `123456`                                    | Mot de passe de première connexion quand aucun hash n'est enregistré                          |
| `DATA_DIR`                                           | `~/.aimix`                                | Emplacement principal des données de l'app (SQLite dans `$DATA_DIR/db/data.sqlite`)          |
...
- `${DATA_DIR}` et `~/.aimix` résolvent tous deux le même emplacement dans un conteneur Docker — le lien symbolique `/root/.aimix -> /app/data` est créé au moment de la compilation.
...
- Renouvelé automatiquement par AIMix
...
Merci à tous les contributeurs qui ont contribué à améliorer AIMix !
...
**[OmniRoute](https://github.com/diegosouzapw/OmniRoute)** — Un fork TypeScript complet de AIMix. Ajoute plus de 36 fournisseurs, un repli automatique à 4 niveaux, des API multimodales (images, embeddings, audio, TTS), un disjoncteur, un cache sémantique, des évaluations de LLM et un tableau de bord soigné. Plus de 368 tests unitaires. Disponible via npm et Docker.
...
- **[RTK](https://github.com/rtk-ai/rtk)** ![Stars](https://img.shields.io/github/stars/rtk-ai/rtk?style=flat&color=yellow) — économiseur de tokens en Rust. AIMix porte son pipeline de compression en JS → **−20 à 40 % de tokens d'entrée** sur chaque requête.
- **[Caveman](https://github.com/JuliusBrussee/caveman)** ![Stars](https://img.shields.io/github/stars/JuliusBrussee/caveman?style=flat&color=yellow) par **[@JuliusBrussee](https://github.com/JuliusBrussee)** — viral _"why use many token when few token do trick"_. AIMix adapte son prompt → **−65 % de tokens de sortie**.
- **[Ponytail](https://github.com/DietrichGebert/ponytail)** ![Stars](https://img.shields.io/github/stars/DietrichGebert/ponytail?style=flat&color=yellow) par **[@DietrichGebert](https://github.com/DietrichGebert)** — compétence de _"dev sénior fainéant"_. AIMix injecte sa hiérarchie YAGNI-d'abord → **moins de tokens, moins de code, des diffs plus courts**.

Un immense merci à ces auteurs — sans leur travail, les fonctionnalités d'économie de tokens de AIMix n'existeraient pas. Mettez-leur une ⭐ sur GitHub !
