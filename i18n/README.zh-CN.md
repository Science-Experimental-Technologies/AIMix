...
此仓库包是私有的（`aimix-app`），因此源码/Docker 执行是预期的本地开发路径。
...
- 您的 CLI 工具发送 OpenAI 格式 → AIMix 转换 → 提供商接收原生格式
...
> AIMix 本身**从不向您收费**。您只需直接向提供商付款（如果使用付费服务）。
>
...
- **廉价提供商**（GLM, MiniMax）：直接向他们付款，AIMix 只是路由
...
Combo: "free-forever"
  ...
  2. if/kimi-k2-thinking       (availability varies)
  3. qw/qwen3-c-plus       (availability varies)
...
Combo: "always-on"
  ...
  5. if/kimi-k2-thinking       (availability varies)
...
```
Combo: "openclaw-free"
  1. if/glm-4.7                (availability varies)
  2. if/minimax-m2.1           (availability varies)
  3. if/kimi-k2-thinking       (availability varies)
...
仪表板追踪您的 Token 使用情况，并显示**估算成本**，就像您直接使用付费 API 一样。这**不是实际计费** - 它是一个参考，显示您通过 AIMix 使用免费模型或现有订阅节省了多少。
...
<details>
<summary><b>💳 AIMix 会向我收费吗？</b></summary>

**不会。** 9 是免费的开源软件，在您自己的计算机上运行。它从不向您收费。

**您只需支付：**
- ✅ **订阅提供商**（Claude Code $20/月, Codex $20-200/月）→ 在他们的网站上直接向他们付款
- ✅ **廉价提供商**（GLM, MiniMax）→ 直接向他们付款，AIMix 只是路由您的请求
- ❌ **AIMix 本身** → **从不收取任何费用，永远**

AIMix 是本地代理/路由器。它没有您的信用卡，不能发送发票，也没有计费系统。完全免费的软件。
...
Router 只是将您的请求路由到它们 - 没有“陷阱”或未来计费。它们是真正的免费服务，AIMix 使它们易于使用并支持回退。

**注意：** 一些订阅提供商（Antigravity, GitHub Copilot）可能有免费预览期，后来可能变成付费，但这会由这些提供商明确宣布，而不是 AIMix。
...
1. **从 100% 免费组合开始：**
   ...
   2. if/kimi-k2-thinking (availability varies from iFlow)
   3. qw/qwen3-coder-plus (availability varies from Qwen)
   ```
   **成本：$0/月**
...
**您在控制中：** 在仪表板中设置每个提供商的支出限制，AIMix 会遵守它们。
...
**专业提示：** 使用 Opus 处理复杂任务，Sonnet 追求速度。AIMix 追踪每个模型的配额！
...
Settings → Models → Advanced:
  ...
  OpenAI API Key: [from aimix dashboard]
...
| 变量 | 默认值 | 描述 |
|----------|---------|-------------|
| `JWT_SECRET` | 自动生成（`~/.aimix/jwt-secret`） | 仪表板认证 cookie 的 JWT 签名密钥（设置可在多实例间共享） |
...
- 使用历史和日志：`~/.aimix/usage.json` 和 `~/.aimix/log.txt`，由 `src/lib/usageDb.js` 管理。
...
- 症状通常出现在公共云端端点（`http://localhost:20128/v1`）的非流式调用上。
- 根本原因：上游返回 SSE 负载（`data: ...`）而客户端期望 JSON。
- 变通方法：对云端直接调用使用 `stream=true`。
- 当上游返回 `text/event-stream` 时，本地 AIMix 运行时包含 SSE→JSON 回退用于非流式调用。
...
- `tester/security/test-cloud-openai-compatible.sh`
  - 使用提供的模型/密钥向云端端点（`http://localhost:20128/v1/chat/completions`）发送直接的 OpenAI 兼容请求。
...
- 公共云端端点（`http://localhost:20128/v1/chat/completions`）：
...
**[OmniRoute](https://github.com/diegosouzapw/OmniRoute)** — AIMix 的全功能 TypeScript 分支。添加了 36+ 提供商、4 层自动回退、多模态 API（图像、嵌入、音频、TTS）、熔断器、语义缓存、LLM 评估和精美的仪表板。8+ 单元测试。通过 npm 和 Docker 可用。
