<div align="center">
  <img src="../images/aimix.png?1" alt="Panel de control de AIMix" width="800"/>

  # AIMix - Enrutador de IA GRATUITO y ahorrador de tokens
...
[🚀 Inicio rápido](#-inicio-rápido) • [💡 Características](#-características-principales) • [📖 Configuración](#-guía-de-instalación) • [🌐 Sitio web](http://localhost:20128)
...
El paquete de este repositorio es privado (`aimix-app`), por lo que ejecutar desde el código fuente/Docker es la ruta de desarrollo local prevista.
...
AIMix funciona a la perfección con todas las principales herramientas de código con IA:
...
Headroom es opcional y se ejecuta por separado. AIMix llama al endpoint local `/v1/compress` de Headroom y luego mantiene el enrutamiento, la reserva, la autenticación y el seguimiento de uso normales:

```
Client → AIMix → Headroom /v1/compress → AIMix → provider
```
...
Si Headroom está caído o devuelve un error, AIMix falla de forma segura y envía la solicitud original.
...
- Tu herramienta CLI envía el formato OpenAI → AIMix lo traduce → El proveedor recibe el formato nativo
...
> AIMix por sí mismo **nunca te cobra** nada. Solo pagas a los proveedores directamente (si usas servicios de pago).
>
...
### 📊 Cómo entender los costes y la facturación de AIMix

**Realidad de la facturación de AIMix:**

✅ **El software de AIMix = GRATUITO para siempre** (código abierto, nunca cobra)
...
❌ **AIMix nunca envía facturas** ni carga tu tarjeta
...
- **Proveedores económicos** (GLM, MiniMax): Págales directamente, AIMix solo enruta
- **Proveedores GRATUITOS** (iFlow, Kiro, Qwen): Realmente gratuitos para siempre, sin cargos ocultos
- **AIMix**: Nunca cobra nada, jamás
...
El panel realiza el seguimiento de tu uso de tokens y muestra **costes estimados** como si estuvieras usando APIs de pago directamente. Esto **no es facturación real** — es una referencia para mostrar cuánto estás ahorrando al usar modelos gratuitos o suscripciones existentes a través de AIMix.
...
<details>
<summary><b>💳 ¿AIMix me cobrará algo?</b></summary>

**No.** AIMix es un software gratuito y de código abierto que se ejecuta en tu propio ordenador. Nunca te cobra nada.

**Solo pagas:**

- ✅ **Proveedores de suscripción** (Claude Code $20/mes, Codex $20-200/mes) → Págales directamente en sus sitios web
- ✅ **Proveedores económicos** (GLM, MiniMax) → Págales directamente, AIMix solo enruta tus solicitudes
- ❌ **El propio AIMix** → **Nunca cobra nada, jamás**

AIMix es un proxy/enrutador local. No tiene tu tarjeta de crédito, no puede enviar facturas y no tiene sistema de facturación. Es software totalmente gratuito.
...
AIMix solo enruta tus solicitudes hacia ellos — no hay "trampa" ni facturación futura. Son servicios realmente gratuitos y AIMix los hace fáciles de usar con soporte de reserva.
...
3. **Usa los proveedores de suscripción al final:**
   ...
   - AIMix te ayuda a maximizar su valor mediante el seguimiento de cuota
...
La reserva inteligente de AIMix evita cargos sorpresa:
...
**Tú tienes el control:** Establece límites de gasto por proveedor en el panel y AIMix los respeta.
...
**Consejo profesional:** Usa Opus para tareas complejas y Sonnet para velocidad. ¡AIMix realiza el seguimiento de la cuota por modelo!
...
Settings → Models → Advanced:
  ...
  OpenAI API Key: [desde el panel de aimix]
...
```json
{
  ...
  "anthropic_api_key": "tu-clave-api-de-aimix"
}
```
...
export OPENAI_API_KEY="tu-clave-api-de-aimix"
...
> **Nota:** OpenClaw solo funciona con AIMix local. Usa `127.0.0.1` en lugar de `localhost` para evitar problemas de resolución IPv6.
...
**Persistencia de datos:** `$HOME/.aimix/db/data.sqlite` en el host ↔ `/app/data/db/data.sqlite` en el contenedor.

### Variables de entorno

| Variable                                             | Por defecto                                | Descripción                                                                          |
| ---------------------------------------------------- | ------------------------------------------ | ------------------------------------------------------------------------------------ |
| `JWT_SECRET`                                         | Generado automáticamente (`~/.aimix/jwt-secret`) | Secreto de firma JWT para la cookie de autenticación del panel (sobreescríbelo para compartir entre instancias) |
| `INITIAL_PASSWORD`                                   | `123456`                                   | Contraseña del primer inicio de sesión cuando no existe un hash guardado             |
| `DATA_DIR`                                           | `~/.aimix`                               | Ubicación principal de datos de la app (SQLite en `$DATA_DIR/db/data.sqlite`)        |
...
- Tanto `${DATA_DIR}` como `~/.aimix` resuelven a la misma ubicación en un contenedor de Docker — el enlace simbólico `/root/.aimix -> /app/data` se crea en tiempo de compilación.
...
- AIMix lo renueva automáticamente
...
¡Gracias a todos los colaboradores que ayudaron a mejorar AIMix!
...
**[OmniRoute](https://github.com/diegosouzapw/OmniRoute)** — Un fork de AIMix en TypeScript con todas las funciones. Añade más de 36 proveedores, reserva automática de 4 niveles, APIs multimodales (imágenes, embeddings, audio, TTS), interruptor de circuitos, caché semántica, evaluaciones de LLM y un panel pulido. Más de 368 pruebas unitarias. Disponible a través de npm y Docker.
...
- **[RTK](https://github.com/rtk-ai/rtk)** ![Stars](https://img.shields.io/github/stars/rtk-ai/rtk?style=flat&color=yellow) — ahorrador de tokens en Rust. AIMix porta su pipeline de compresión a JS → **−20-40% de tokens de entrada** en cada solicitud.
- **[Caveman](https://github.com/JuliusBrussee/caveman)** ![Stars](https://img.shields.io/github/stars/JuliusBrussee/caveman?style=flat&color=yellow) por **[@JuliusBrussee](https://github.com/JuliusBrussee)** — viral _"why use many token when few token do trick"_. AIMix adapta su prompt → **−65% de tokens de salida**.
- **[Ponytail](https://github.com/DietrichGebert/ponytail)** ![Stars](https://img.shields.io/github/stars/DietrichGebert/ponytail?style=flat&color=yellow) por **[@DietrichGebert](https://github.com/DietrichGebert)** — habilidad del _"dev sénior perezoso"_. AIMix inyecta su jerarquía primero-YAGNI → **menos tokens, menos código, diffs más cortos**.

Un enorme agradecimiento a estos autores — sin su trabajo, las funciones de ahorro de tokens de AIMix no existirían. ¡Dales una ⭐ en GitHub!
