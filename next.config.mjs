...
const tracingRoot = process.env.NEXT_TRACING_ROOT_MODE === "workspace"
  ? join(projectRoot, "..")
  : projectRoot;
const proxyClientMaxBodySize = process.env.AIMIX_PROXY_CLIENT_MAX_BODY_SIZE || "128mb";
const disableFileTracing = process.env.AIMIX_DISABLE_FILE_TRACING === "true";

/** @type {import('next').NextConfig} */
const nextConfig = {
  distDir: process.env.NEXT_DIST_DIR || ".next",
  output: disableFileTracing ? undefined : "standalone",
  ...
  outputFileTracingExcludes: {
    // IDE credential auto-import routes inspect these locations only at runtime.
    // Excluding them prevents Next's build tracer from recursively following
    // user-profile symlinks (including stale VS Code askpass links on Windows).
    "*": ["./gitbook/**/*", "**/AppData/**", "**/.config/**", "**/.aws/**"]
  },
  images: {
    unoptimized: true
  },
