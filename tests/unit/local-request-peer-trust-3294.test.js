...
// path, so x-aimix-real-ip arrives straight from the client and a remote caller can claim to
...
describe("peer header trust", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    process.env.AIMIX_PEER_TOKEN = PEER_TOKEN;
  ...
  afterEach(() => {
    ...
    delete process.env.AIMIX_PEER_TOKEN;
  });
  ...
  it("rejects a spoofed loopback peer IP carrying a wrong trust token", async () => {
    const response = await proxy(request("/api/v1/models", {
      host: "172.18.192.1:20140",
      "x-aimix-real-ip": "127.0.0.1",
      "x-aimix-peer-token": "guessed-token",
    }));
...
describe("login limiter client IP", () => {
  ...
  it("buckets spoofed peer IPs together so lockout cannot be rotated away", () => {
    const first = getClientIp(request("/api/auth/login", { "x-aimix-real-ip": "1.1.1.1" }));
    const second = getClientIp(request("/api/auth/login", { "x-aimix-real-ip": "2.2.2.2" }));
  ...
  it("still honours TRUST_PROXY for operators fronting aimix with a reverse proxy", () => {
