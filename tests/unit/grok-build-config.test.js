...
describe("grokBuildConfig", () => {
  it("creates independent main and per-type subagent model slots", () => {
    ...
    expect(parsed.default).toBe("aimix");
    ...
    expect(parsed.subagentMappings).toMatchObject({
      "general-purpose": "aimix-general-purpose",
      explore: "aimix-explore",
      plan: "grok-4.5",
    });
  ...
  it("is idempotent and updates owned slots without duplicate sections", () => {
    ...
    expect(result.match(/^\[model\.aimix\]$/gm)).toHaveLength(1);
    expect(result.match(/^\[model\.aimix-general-purpose\]$/gm)).toHaveLength(1);
    expect(result.match(/^\[model\.aimix-explore\]$/gm)).toHaveLength(1);
    expect(result.match(/^# aimix-prev-subagent-explore/gm)).toHaveLength(1);
  ...
  it("blank override restores previous subagent mapping and removes owned slot", () => {
    ...
    expect(result).not.toContain("[model.aimix-explore]");
    expect(parsed.subagentMappings["general-purpose"]).toBe("aimix-general-purpose");
  });
  ...
  it("reset restores previous default and all previous subagent mappings", () => {
    ...
    expect(reset).not.toContain("[model.aimix-");
    expect(reset).not.toContain("aimix-prev-");
  ...
  it("removes mappings that were originally unset", () => {
    ...
    expect(parseGrokBuildConfig(applied).subagentMappings.plan).toBe("aimix-plan");
  ...
  it("legacy callers without subagentModels leave existing overrides untouched", () => {
    ...
    expect(parsed.subagentMappings.explore).toBe("aimix-explore");
  ...
  it("returns stable slot names only for supported subagent types", () => {
    expect(getGrokSubagentSlot("general-purpose")).toBe("aimix-general-purpose");
    expect(getGrokSubagentSlot("explore")).toBe("aimix-explore");
    expect(getGrokSubagentSlot("plan")).toBe("aimix-plan");
