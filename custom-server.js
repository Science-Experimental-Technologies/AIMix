...
// Per-process secret proving x-aimix-real-ip was stamped below rather than sent by the client.
// A bare `next start` / `next dev` never loads this file, so it cannot produce a matching
// header even though the env var is inherited by child processes. Named like x-aimix-cli-token
...
http.createServer = (...args) => {
  ...
  const wrapped = (req, res) => {
    ...
    delete req.headers["x-aimix-real-ip"];
    delete req.headers["x-forwarded-for"];
    delete req.headers["x-aimix-via-proxy"];
    delete req.headers["x-aimix-peer-token"];
    req.headers["x-aimix-real-ip"] = ip;
    req.headers["x-aimix-peer-token"] = PEER_TOKEN;
    if (viaProxy) req.headers["x-aimix-via-proxy"] = "1";
