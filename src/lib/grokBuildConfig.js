...
const previousDefaultRegExp = /^# aimix-prev-default = "([^"]*)"[ \t]*\r?\n?/m;
const previousSubagentRegExp = (type) =>
  new RegExp(
    `^# aimix-prev-subagent-${escapeRegExp(type)} = "([^"]*)"[ \\t]*\\r?\\n?`,
    "m",
  );
...
function buildModelSection({ slot, model, baseUrl, apiKey, contextWindow, name }) {
  const lines = [
    ...
    `description = ${tomlString("Routed via AIMix gateway")}`,
...
function rememberPreviousDefault(toml) {
  ...
  return insertMarker(toml, `# aimix-prev-default = ${tomlString(current)}\n`);
}
...
function rememberPreviousSubagent(toml, type) {
  ...
  return insertMarker(
    toml,
    `# aimix-prev-subagent-${type} = ${tomlString(previous)}\n`,
  );
}
