// x-aimix-real-ip is only trustworthy when custom-server.js stamped it from the TCP socket.
...
export function hasTrustedPeerHeaders(request) {
  const token = process.env.AIMIX_PEER_TOKEN;
  return Boolean(token) && request.headers.get("x-aimix-peer-token") === token;
}
