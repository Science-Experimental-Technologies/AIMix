...
// Collect PIDs of all aimix-related processes (excluding current)
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach(line => {
        ...
        // Match anything running from aimix install dir or wrapper cli.js
        const isAppProcess = lower.includes("aimix") ||
    ...
    try {
      ...
      output.split("\n").forEach(line => {
        const isAppProcess = line.includes("aimix") ||
...
function getDataDir() {
  ...
  if (process.platform === "win32") {
    return path.join(process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming"), "aimix");
  }
  return path.join(os.homedir(), ".aimix");
}
...
// Resolve npx/aimix binary to relaunch after update (cross-platform)
function resolveRelaunchCommand() {
  const isWin = process.platform === "win32";
  // Prefer `npx aimix` — works regardless of global bin path changes after npm i -g
