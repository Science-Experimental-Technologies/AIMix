...
// Check if settings has AIMix customModels
const hasAIMixConfig = (settings) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(m => m.id?.startsWith("custom:AIMix"));
};
...
// POST - Update AIMix customModels (merge with existing settings)
...
export async function POST(request) {
  try {
    ...
    // Remove all existing AIMix configs
    settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:AIMix"));
...
// DELETE - Remove AIMix customModels only (keep other settings)
