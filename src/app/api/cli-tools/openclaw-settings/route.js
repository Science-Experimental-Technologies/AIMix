...
// Check if settings has AIMix config
const hasAIMixConfig = (settings) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["aimix"];
};

// Read per-agent models.json and return current model id (without "aimix/" prefix)
const readAgentModel = async (agentDir) => {
  try {
    const modelsPath = path.join(agentDir, "models.json");
    const content = await fs.readFile(modelsPath, "utf-8");
    const data = JSON.parse(content);
    const models = data?.providers?.["aimix"]?.models;
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasAIMix: hasAIMixConfig(settings),
...
// POST - Update AIMix settings (merge with existing settings)
export async function POST(request) {
  try {
    ...
    const fullModelId = `aimix/${model}`;

    // Remove all old aimix/* entries from agents.defaults.models
    Object.keys(settings.agents.defaults.models)
      .filter((k) => k.startsWith("aimix/"))
    ...
    // Add fresh aimix models to allowlist
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`aimix/${m}`] = {};
    });

    // Remove old aimix model from each agent in agents.list. The
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        if (resolveAgentModel(agent.model).startsWith("aimix/")) {
    ...
    // Update models.providers.aimix with all models
    settings.models.providers["aimix"] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        ...
        if (agentModel) return { ...agent, model: `aimix/${agentModel}` };
        return agent;
      });
...
// DELETE - Remove AIMix settings only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove AIMix from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["aimix"];
    ...
    // Remove aimix models from agents.defaults.models allowlist
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => k.startsWith("aimix/"));
    ...
    // Reset agents.defaults.model.primary if it uses aimix
    if (settings.agents?.defaults?.model?.primary?.startsWith("aimix/")) {
    ...
    return NextResponse.json({
      success: true,
      message: "AIMix settings removed successfully",
    });
  } catch (error) {
