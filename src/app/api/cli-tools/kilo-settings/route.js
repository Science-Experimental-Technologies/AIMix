...
const hasAIMixConfig = (auth) => {
  if (!auth) return false;
  const entry = auth["openai-compatible"] || auth["aimix"];
  if (!entry) return false;
  const baseUrl = entry.baseUrl || entry.baseURL || "";
  return baseUrl.includes("localhost") || baseUrl.includes("127.0.0.1") || baseUrl.includes("aimix");
};
...
export async function POST(request) {
  try {
    ...
    try {
      ...
      vscode["kilocode.customProvider"] = { name: "AIMix", baseURL: normalizedBaseUrl, apiKey };
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({ success: true, message: "AIMix settings removed from Kilo Code" });
  } catch (error) {
