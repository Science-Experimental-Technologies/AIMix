...
const getConfigPath = () => {
  ...
  if (platform === "win32") {
    // Read dynamically so Next's build tracer does not crawl the developer's
    // live VS Code globalStorage directory and follow stale extension symlinks.
    const appData = Reflect.get(process.env, ["APP", "DATA"].join(""));
    return path.join(appData || home, "Code", "User", "chatLanguageModels.json");
  }
  if (platform === "darwin") {
...
const hasAIMixConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "AIMix" || entry.name === "AIMix");
};

const getAIMixEntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "AIMix" || entry.name === "AIMix") || null;
};
...
export async function GET() {
  try {
    ...
    const entry = getAIMixEntry(config);
...
// POST - Apply AIMix config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    // Replace existing AIMix entry or append
    const idx = config.findIndex((e) => e.name === "AIMix" || e.name === "AIMix");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove AIMix entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "AIMix" && e.name !== "AIMix");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "AIMix removed from Copilot config",
    });
  } catch (error) {
