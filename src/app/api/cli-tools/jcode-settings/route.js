...
const getProviderEnvPath = () => {
  ...
  return path.join(configDir, "jcode", "provider-aimix.env");
};
...
const hasAIMixConfig = (config) => {
  ...
  if (providers["aimix"]) return true;
...
export async function GET() {
  ...
  const hasAIMix = hasAIMixConfig(config);
...
export async function POST(request) {
  try {
    ...
    env.JCODE_AIMIX_API_KEY = apiKey;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "jcode configured successfully. Use: jcode --provider-profile aimix",
      configPath: getConfigPath(),
    });
  } catch (error) {
...
export async function DELETE() {
  try {
    ...
    delete config.providers["aimix"];

    await writeConfig(config);

    const env = await readProviderEnv();
    delete env.JCODE_AIMIX_API_KEY;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "aimix configuration removed from jcode",
    });
  } catch (error) {
