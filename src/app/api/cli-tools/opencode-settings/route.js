...
const hasAIMixConfig = (config) => {
  if (!config?.provider) return false;
  return !!config.provider["aimix"];
};
...
export async function GET() {
  try {
    ...
    const providerConfig = config?.provider?.["aimix"];
    const modelMap = providerConfig?.models || {};

    return NextResponse.json({
      installed: true,
      config,
      hasAIMix: hasAIMixConfig(config),
      configPath: getConfigPath(),
        opencode: {
          models: Object.keys(modelMap),
          activeModel: config?.model?.startsWith("aimix/") ? config.model.replace(/^aimix\//, "") : null,
...
// POST - Apply AIMix as openai-compatible provider (multi-model support)
export async function POST(request) {
  try {
    ...
    const keyToUse = apiKey || "sk_aimix";
    const effectiveSubagentModel = subagentModel || modelsArray[0];

    // Ensure provider object
    if (!config.provider) config.provider = {};

    // Preserve any existing aimix provider entry and its models
    const existingProvider = config.provider["aimix"] || { npm: "@ai-sdk/openai-compatible", options: {}, models: {} };
    ...
    config.provider["aimix"] = existingProvider;
    ...
    if (activeModel === "") {
      ...
      if (finalActive) {
        config.model = `aimix/${finalActive}`;
      }
    }
    ...
    config.agent.explorer = {
      ...
      mode: "subagent",
      model: `aimix/${effectiveSubagentModel}`,
    };
...
export async function PATCH(request) {
  try {
    ...
    if (clearActiveModel === true) {
      ...
      if (config.model?.startsWith("aimix/")) {
        config.model = "";
      }
    }
...
// DELETE - Remove AIMix provider or specific models from config
export async function DELETE(request) {
  try {
    ...
    if (modelToRemove && config.provider?.["aimix"]?.models) {
      delete config.provider["aimix"].models[modelToRemove];
      
      // If no models left, remove the provider
      if (Object.keys(config.provider["aimix"].models).length === 0) {
        delete config.provider["aimix"];
        if (config.model?.startsWith("aimix/")) delete config.model;
      } else if (config.model === `aimix/${modelToRemove}`) {
        // If removed model was active, switch to first remaining model
        const remainingModels = Object.keys(config.provider["aimix"].models);
        config.model = `aimix/${remainingModels[0]}`;
      }
    } else {
      // No specific model - remove entire aimix provider
      if (config.provider) delete config.provider["aimix"];
      if (config.model?.startsWith("aimix/")) delete config.model;
    }

    // Remove subagent configuration
    if (config.agent?.explorer?.model?.startsWith("aimix/")) {
    ...
    return NextResponse.json({
      success: true,
      message: modelToRemove ? `Model "${modelToRemove}" removed` : "AIMix settings removed from OpenCode",
    });
  } catch (error) {
