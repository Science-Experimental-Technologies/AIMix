...
export default function OpenCodeToolCard({ tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders, cloudEnabled, initialStatus, tunnelEnabled, tunnelPublicUrl, tailscaleEnabled, tailscaleUrl }) {
  ...
  useEffect(() => {
    ...
    if (status?.config?.agent?.explorer?.model?.startsWith("aimix/")) {
      setSubagentModel(status.config.agent.explorer.model.replace("aimix/", ""));
    }
  }, [status]);
  ...
  const saveModels = async (models) => {
    try {
      const keyToUse = (selectedApiKey && selectedApiKey.trim())
        ? selectedApiKey
        : (!cloudEnabled ? "sk_aimix" : selectedApiKey);
  ...
  const getConfigStatus = () => {
    ...
    if (!status.hasAIMix) return "not_configured";
    const url = status.config?.provider?.["aimix"]?.options?.baseURL || "";
          ...
          {!checking && status?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {/* Current configured */}
                {status?.config?.provider?.["aimix"]?.options?.baseURL && (
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button variant="outline" size="sm" onClick={handleReset} disabled={!status.hasAIMix} loading={restoring}>
