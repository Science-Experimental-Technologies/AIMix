---
name: aimix-tts
description: Text-to-speech via AIMix /v1/audio/speech using OpenAI / ElevenLabs / Deepgram / Edge TTS / Google TTS / Hyperbolic / Inworld voices. Use when the user wants to convert text to speech, generate audio, voiceover, narrate, or read text aloud.
---

# AIMix — Text-to-Speech
...
```bash
# 1) List models
curl $AIMIX_URL/v1/models/tts | jq '.data[].id'
# 2) Per-model metadata (params, voicesUrl if voice-by-id)
curl "$AIMIX_URL/v1/models/info?id=el/eleven_multilingual_v2"
# 3) List voices (elevenlabs, edge-tts, deepgram, inworld, local-device). Optional ?lang=vi
curl "$AIMIX_URL/v1/audio/voices?provider=edge-tts&lang=vi" | jq '.data[].model'
```
...
`POST $AIMIX_URL/v1/audio/speech`
...
```bash
curl -X POST "$AIMIX_URL/v1/audio/speech" \
...
const r = await fetch(`${process.env.AIMIX_URL}/v1/audio/speech`, {
  method: "POST",
  headers: { "Authorization": `Bearer ${process.env.AIMIX_KEY}`, "Content-Type": "application/json" },
