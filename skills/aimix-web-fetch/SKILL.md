---
name: aimix-web-fetch
description: Fetch URL → markdown / text / HTML via AIMix /v1/web/fetch using Firecrawl / Jina Reader / Tavily Extract / Exa Contents. Use when the user wants to scrape a webpage, extract URL content, read article, or convert a URL to markdown.
---
...
```bash
curl $AIMIX_URL/v1/models/web | jq '.data[] | select(.kind=="webFetch") | .id'
# Per-provider params
curl "$AIMIX_URL/v1/models/info?id=firecrawl/fetch"
```
...
### Jina Reader
```bash
curl -X POST $AIMIX_URL/v1/web/fetch \
...
```js
const r = await fetch(`${process.env.AIMIX_URL}/v1/web/fetch`, {
  method: "POST",
