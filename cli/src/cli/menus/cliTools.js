...
async function buildCodexHeader() {
  ...
  const { installed, hasAIMix, config } = result.data;
...
async function buildDroidHeader() {
  ...
  const { installed, hasAIMix, settings } = result.data;
  ...
  // Extract AIMix custom model config
  const custom = settings?.customModels?.find(m => m.id === "custom:AIMix-0");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenClawHeader() {
  ...
  // Extract AIMix provider config
  const provider = settings?.models?.providers?.["aimix"];
  const primary = settings?.agents?.defaults?.model?.primary || "";
  const model = primary.startsWith("aimix/") ? primary.replace("aimix/", "") : (provider?.models?.[0]?.id || "");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenCodeHeader() {
  ...
  const { installed, hasAIMix, opencode } = result.data;
...
async function showCliToolsMenu(port, breadcrumb = []) {
  ...
  await showMenuWithBack({
    title: "🔧 CLI Tools",
    breadcrumb,
    headerContent: `Configure CLI tools to use AIMix\nEndpoint: ${endpoint}`,
    items: [
      {
        label: "Claude Code",
