...
const aimixControl = require("./src/cli/commands/aimixControl");
if (aimixControl.commands.has(args[0])) {
  aimixControl.run(args[0], args.slice(1)).then((code) => process.exit(code)).catch((error) => { console.error(`❌ ${error.message}`); process.exit(1); });
  return;
}

// Subcommands (`aimix xai video …`) run against an already-running gateway
...
// Self-heal SQLite runtime deps (sql.js + better-sqlite3) into ~/.aimix/runtime
...
// Identifiers for killAllAppProcesses - only kill aimix specifically
const PROCESS_IDENTIFIERS = [
  'aimix'  // Only package name - avoid killing other apps
];
...
function getAppDataDir() {
  return process.platform === "win32"
    ? path.join(process.env.APPDATA || "", "aimix")
    : path.join(os.homedir(), ".aimix");
}
    ...
    try {
      ...
      if (platform === "win32") {
        ...
        try {
          ...
          const lines = output.split("\n").slice(1).filter(l => l.trim());
          lines.forEach(line => {
            // Whitelist: real node process running aimix/cli.js, or next-server.
            // Avoids killing editors/grep/strace/cursor that just have "aimix" in cmdline.
            const cmd = line.toLowerCase();
            const isAppProcess =
              (cmd.includes("node") && cmd.includes("aimix") && (cmd.includes("cli.js") || cmd.includes("\\aimix") || cmd.includes("/aimix")))
        ...
        try {
          ...
          lines.forEach(line => {
            // Whitelist: real node process running aimix/cli.js, or next-server.
            // Avoids killing grep/strace/editors/cursor that incidentally match "aimix".
            const cmd = line.toLowerCase();
            const isAppProcess =
              (cmd.includes("node") && cmd.includes("aimix") && (cmd.includes("cli.js") || cmd.includes("/aimix")))
              || cmd.includes("next-server");
            if (isAppProcess) {
              const parts = line.trim().split(/\s+/);
              const pid = parts[1];
...
function startServer(updatePromise) {
      ...
      while (true) {
        ...
        if (choice === "update") {
          ...
          if (process.platform === "darwin") {
            ...
            console.log(`🔔 AIMix is running in tray (PID: ${process.pid})`);
            console.log(`   Server: http://${displayHost}:${port}`);
          ...
          console.log(`🔔 AIMix is now running in background (PID: ${bgProcess.pid})`);
          console.log(`   Server: http://${displayHost}:${port}`);
          console.log(`\n💡 You can close this terminal. Right-click tray icon to quit.\n`);
  ...
  function tryRestart(code) {
    ...
    if (restartCount >= MAX_RESTARTS) {
      ...
      try {
        const dbPath = path.join(os.homedir(), process.platform === "win32" ? path.join("AppData", "Roaming", "aimix", "db.json") : path.join(".aimix", "db.json"));
