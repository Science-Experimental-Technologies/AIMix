...
const cliAppDir = process.env.AIMIX_CLI_APP_DIR || path.join(cliDir, "app");
...
function resolveStandaloneBuild(appDir, buildDistDir) {
  ...
  // NEXT_TRACING_ROOT_MODE=workspace, e.g. standalone/aimix/server.js.
...
function buildCliPackage() {
  console.log("📦 Building AIMix CLI package with Next.js...\n");
  ...
  // Strip better-sqlite3 (native) — it lives in ~/.aimix/runtime to avoid
  ...
  if (fs.existsSync(betterDir)) {
    ...
    console.log("✅ Stripped better-sqlite3 (lives in ~/.aimix/runtime)");
  }
  console.log("");
